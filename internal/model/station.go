...
func (a *Station) ShowSelect() []int {
	...
	for trackNum, track := range a.Tracks {
		data = append(data, []string{
			fmt.Sprint(trackNum + 1),
			fmt.Sprintf("%02d. %s", track.Resp.Attributes.TrackNumber, track.Name),
	...
	for _, part := range strings.Split(input, ",") {
		...
		if len(option) == 1 {
			num, err := strconv.Atoi(option[0])
			if err != nil {
				fmt.Println("Invalid option:", option[0])
				continue
			}
			if num > 0 && num <= len(arr) {
				selected = append(selected, num)
			} else {
				fmt.Println("Option out of range:", option[0])
			}
			continue
		}
		if len(option) != 2 {
		...
		start, err1 := strconv.Atoi(option[0])
		end, err2 := strconv.Atoi(option[1])
