...
export default function ApplePurgePanel({ compact = false }) {
  ...
  const confirmAndRun = async (elevated) => {
    const confirmed = await requestConfirm({
      ...
      message: elevated
        ...
        : 'Closes Apple Music, clears PC artwork caches (UWP app, library bundle art, legacy iTunes folders), and tries to restart the mobile device service. Your .m4a download files are not deleted. iPhone artwork is not cleared on the device.',
  ...
  return (
    <section
      className={
        compact
          ? 'rounded-xl border border-amber-500/20 bg-amber-500/[0.04] p-3'
          : 'mt-4 space-y-3 rounded-xl border border-amber-500/20 bg-amber-500/[0.04] p-4'
      }
    >
      <div>
        <h3 className={`font-medium text-white/90 ${compact ? 'text-sm' : ''}`}>Apple Music PC cache purge</h3>
        <p className="mt-1 text-xs leading-relaxed text-white/50">
          {compact
            ? 'Wrong album art on PC after fixing tags? Stronger than Tag Editor cache clear — quits Apple Music and wipes PC artwork caches.'
            : 'Use when Apple Music on this PC keeps showing wrong album art after you fixed tags, or when Tag Editor sync repair is not enough.'}
        </p>
      </div>
      ...
      {!compact && (
        <ul className="list-inside list-disc space-y-1 text-[11px] leading-relaxed text-white/45">
          ...
          <li>Clears UWP app caches, library bundle artwork, and legacy iTunes artwork folders.</li>
          ...
          <li>Afterward: re-open Apple Music and re-import your albums.</li>
        </ul>
      )}
      ...
      {result && (
        ...
        >
          ...
          {result.manual_steps?.length > 0 && !compact && (
